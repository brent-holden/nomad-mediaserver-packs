...
variable "image" {
  description = "The container image to use for Tautulli"
  type        = string
  default     = "docker.io/linuxserver/tautulli:latest"
}

variable "tautulli_uid" {
  description = "The UID for the Tautulli user inside the container (PUID)"
  type        = number
  default     = 1000
}

variable "tautulli_gid" {
  description = "The GID for the Tautulli group inside the container (PGID)"
  type        = number
  default     = 1000
}

variable "timezone" {
  description = "The timezone for the Tautulli container"
  type        = string
...
variable "port" {
  description = "The port to expose Tautulli on"
  type        = number
  default     = 8181
}

variable "config_volume_name" {
  description = "The name of the host volume for Tautulli configuration"
  type        = string
  default     = "tautulli-config"
}

variable "register_consul_service" {
  description = "Register the Tautulli service with Consul"
  type        = bool
  default     = true
}
...
variable "enable_backup" {
  description = "Enable periodic backup job for Tautulli configuration"
  type        = bool
  default     = true
}
