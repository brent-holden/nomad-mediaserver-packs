...
if [ -f "$RESTORE_SOURCE/tautulli.db" ]; then
    echo "Restoring Tautulli database..."
    cp "$RESTORE_SOURCE/tautulli.db" "$CONFIG_DIR/"
    echo "Database restore complete."
else
    echo "Warning: No tautulli.db found in backup"
fi

# Restore config file
if [ -f "$RESTORE_SOURCE/config.ini" ]; then
    echo "Restoring config.ini..."
    cp "$RESTORE_SOURCE/config.ini" "$CONFIG_DIR/"
    echo "Config restore complete."
else
    echo "Warning: No config.ini found in backup"
fi

# Restore internal backups
if [ -d "$RESTORE_SOURCE/backups" ]; then
    echo "Restoring Tautulli internal backups..."
...
chown -R [[ var "tautulli_uid" . ]]:[[ var "tautulli_gid" . ]] "$CONFIG_DIR" 2>/dev/null || echo "Warning: Could not set ownership"
