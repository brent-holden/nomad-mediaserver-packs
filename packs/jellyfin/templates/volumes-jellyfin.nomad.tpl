...
curl -sf -X PUT "$NOMAD_ADDR/v1/volume/host/[[ var "cache_volume_name" . ]]/create" \
  -H "Content-Type: application/json" \
  -d @/tmp/cache-volume.json && echo " -> Success" || echo " -> Already exists or failed"
...
cat > /tmp/media-volume.json << MEDIA_EOF
{
  ...
  "MountOptions": {
    "FSType": "cifs",
    "MountFlags": ["uid=[[ var "jellyfin_uid" . ]]", "gid=[[ var "jellyfin_gid" . ]]", "file_mode=0644", "dir_mode=0755", "vers=3.0"]
  },
  "Secrets": {
...
cat > /tmp/backup-volume.json << BACKUP_EOF
{
  ...
  "MountOptions": {
    "FSType": "cifs",
    "MountFlags": ["uid=[[ var "jellyfin_uid" . ]]", "gid=[[ var "jellyfin_gid" . ]]", "file_mode=0644", "dir_mode=0755", "vers=3.0", "cache=none", "nobrl"]
  },
  "Secrets": {
    "username": "[[ var "csi_volume_username" . ]]",
    "password": "[[ var "csi_volume_password" . ]]"
