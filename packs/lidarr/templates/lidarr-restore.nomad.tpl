...
if [ -f "$RESTORE_SOURCE/lidarr.db" ]; then
    echo "Restoring Lidarr database..."
    cp "$RESTORE_SOURCE/lidarr.db" "$CONFIG_DIR/"
    echo "Database restore complete."
else
    echo "Warning: No lidarr.db found in backup"
fi
...
if [ -d "$RESTORE_SOURCE/Backups" ]; then
    echo "Restoring Lidarr internal backups..."
...
chown -R [[ var "lidarr_uid" . ]]:[[ var "lidarr_gid" . ]] "$CONFIG_DIR" 2>/dev/null || echo "Warning: Could not set ownership"
