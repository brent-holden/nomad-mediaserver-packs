...
echo "Starting Lidarr backup job..."
...
if [ -f "/lidarr-config/lidarr.db" ]; then
    echo "Backing up Lidarr database..."
    cp "/lidarr-config/lidarr.db" "$BACKUP_DEST/"
fi

if [ -f "/lidarr-config/config.xml" ]; then
    echo "Backing up config.xml..."
    cp "/lidarr-config/config.xml" "$BACKUP_DEST/"
fi

if [ -d "/lidarr-config/Backups" ]; then
    echo "Backing up Lidarr internal backups..."
    rsync -av "/lidarr-config/Backups/" "$BACKUP_DEST/Backups/"
fi
