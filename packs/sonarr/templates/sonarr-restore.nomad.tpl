...
if [ -f "$RESTORE_SOURCE/sonarr.db" ]; then
    echo "Restoring Sonarr database..."
    cp "$RESTORE_SOURCE/sonarr.db" "$CONFIG_DIR/"
    echo "Database restore complete."
else
    echo "Warning: No sonarr.db found in backup"
fi
...
if [ -d "$RESTORE_SOURCE/Backups" ]; then
    echo "Restoring Sonarr internal backups..."
...
chown -R [[ var "sonarr_uid" . ]]:[[ var "sonarr_gid" . ]] "$CONFIG_DIR" 2>/dev/null || echo "Warning: Could not set ownership"
