...
if [ -f "$RESTORE_SOURCE/radarr.db" ]; then
    echo "Restoring Radarr database..."
    cp "$RESTORE_SOURCE/radarr.db" "$CONFIG_DIR/"
    echo "Database restore complete."
else
    echo "Warning: No radarr.db found in backup"
fi
...
if [ -d "$RESTORE_SOURCE/Backups" ]; then
    echo "Restoring Radarr internal backups..."
...
chown -R [[ var "radarr_uid" . ]]:[[ var "radarr_gid" . ]] "$CONFIG_DIR" 2>/dev/null || echo "Warning: Could not set ownership"
