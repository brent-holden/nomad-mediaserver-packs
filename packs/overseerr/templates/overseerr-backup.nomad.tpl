...
echo "Starting Overseerr backup job..."

apt-get update -qq && apt-get install -y -qq rsync > /dev/null 2>&1

BACKUP_DIR="/backups/overseerr"
DATE=$(date +%Y-%m-%d)
BACKUP_DEST="$BACKUP_DIR/$DATE"

mkdir -p "$BACKUP_DEST"

if [ -f "/overseerr-config/db/db.sqlite3" ]; then
    echo "Backing up Overseerr database..."
    mkdir -p "$BACKUP_DEST/db"
    cp "/overseerr-config/db/db.sqlite3" "$BACKUP_DEST/db/"
fi

if [ -f "/overseerr-config/settings.json" ]; then
    echo "Backing up settings.json..."
    cp "/overseerr-config/settings.json" "$BACKUP_DEST/"
fi
